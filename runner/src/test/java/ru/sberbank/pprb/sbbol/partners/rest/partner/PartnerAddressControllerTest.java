...
public class PartnerAddressControllerTest extends AbstractIntegrationTest {
    ...
    void testGetPartnerAddress() {
        ...
        var address = createValidAddress(partner.getUuid());
    ...
    void testViewPartnerAddress() {
        ...
        createValidAddress(partner.getUuid(), partner.getDigitalId());
        createValidAddress(partner.getUuid(), partner.getDigitalId());
        createValidAddress(partner.getUuid(), partner.getDigitalId());
        createValidAddress(partner.getUuid(), partner.getDigitalId());
    ...
    void testUpdatePartnerAddress() {
        ...
        updateAddress.uuid(address.getUuid());
        updateAddress.digitalId(address.getDigitalId());
        updateAddress.unifiedUuid(address.getUnifiedUuid());
    ...
    void testDeletePartnerAddress() {
        ...
        var deleteAddress =
            delete(
                baseRoutePath + "/address" + "/{digitalId}" + "/{id}",
                Error.class,
                actualAddress.getAddress().getDigitalId(), actualAddress.getAddress().getUuid()
            );
    ...
    private static Address createValidAddress(String partnerUuid, String digitalId) {
        var response = post(baseRoutePath + "/address", getValidPartnerAddress(partnerUuid, digitalId), AddressResponse.class);
        assertThat(response)
            .isNotNull();
    ...
    private static Address createValidAddress(String partnerUuid) {
        var response = post(baseRoutePath + "/address", getValidPartnerAddress(partnerUuid), AddressResponse.class);
        assertThat(response)
            .isNotNull();
        assertThat(response.getErrors())
            .isNull();
