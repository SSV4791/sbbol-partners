...
public class ContactEmailControllerTest extends AbstractIntegrationWithOutSbbolTest {
    ...
    void testCreateContactEmail() {
        var partner = createValidPartner(RandomStringUtils.randomAlphabetic(10));
        var contact = createValidContact(partner.getId(), partner.getDigitalId());
        var email = createEmail(contact.getId());
        assertThat(email)
    ...
    private static Email getEmail(String contactUuid) {
        return new Email()
            .version(0L)
    ...
    private static Email createEmail(String contactUuid) {
        var emailResponse = createPost(baseRoutePath, getEmail(contactUuid), EmailResponse.class);
